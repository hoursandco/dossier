...
export interface DealThumbnailCropSelection {
  accepted: boolean
  image_url: string
  crop: {
    x: number
...
const DealThumbnailCropSelectionSchema = z.object({
  accepted: z.boolean(),
  image_url: z.string(),
  crop: z.object({
    x: z.number(),
...
const dealThumbnailCropJsonSchema: JsonSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['accepted', 'image_url', 'crop', 'confidence', 'alt_text', 'reason'],
  properties: {
...
function buildDealThumbnailCropRequest(
  deal: {
    retailer: string
    description: string
    deal_type: string
  ...
  return {
    systemPrompt: `You select product thumbnails for Deal Dossier sale cards.

Choose one supplied email image only if it contains a real product photo that matches the deal. Return a normalized crop rectangle around the product itself, excluding sale text, coupon text, logos, navigation, app badges, borders, and decorative background when possible.

Reject images that are primarily text banners, logos, lifestyle scenes without a clear product, tracking pixels, or unrelated products. Normalized crop values must be from 0 to 1 relative to the chosen image. Prefer a slightly generous product crop over cutting off the item. Return JSON exactly matching the schema.`,
    userContent: [
      {
        type: 'text',
        text: `DEAL
...
If no image is a good product match, set accepted=false, image_url="", crop={x:0,y:0,width:1,height:1}, confidence=0.`,
      },
      ...urls.map((url) => ({
        type: 'image_url' as const,
        image_url: { url },
    ...
    schemaName: 'deal_thumbnail_crop',
    schema: dealThumbnailCropJsonSchema,
  }
}
...
export async function selectDealThumbnailCrop(
  deal: {
    retailer: string
    description: string
    deal_type: string
  ...
  imageUrls: string[],
): Promise<DealThumbnailCropSelection | null> {
  const request = buildDealThumbnailCropRequest(deal, imageUrls)
  ...
  try {
    ...
    const validated = DealThumbnailCropSelectionSchema.safeParse(parsed)
    if (!validated.success) {
      console.warn('[deal-thumbnail] crop selection schema failed:', JSON.stringify(validated.error.issues))
      return null
    }

    const selection = validated.data
    if (!selection.accepted || !usableImageUrls(imageUrls).includes(selection.image_url)) {
      return null
    }
    ...
    return {
      ...
      crop: {
        x: clamp01(selection.crop.x),
        y: clamp01(selection.crop.y),
        width: clamp01(selection.crop.width),
        height: clamp01(selection.crop.height),
      },
      confidence: clamp01(selection.confidence),
      alt_text: normalizeText(selection.alt_text).slice(0, 140),
      reason: normalizeText(selection.reason).slice(0, 240),
    }
  } catch (err) {
    if (err instanceof ApiCreditExhaustedError || err instanceof ApiRateLimitError) throw err
    console.warn('[deal-thumbnail] crop selection failed:', err)
    return null
  }
}
